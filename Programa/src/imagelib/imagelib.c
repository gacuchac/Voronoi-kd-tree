  ...
  png_read_image(png, row_pointers);
	...
	for(int row = 0; row < height; row++)
	{
		...
		for(int col = 0; col < width; col++)
		{
			...
			img -> pixels[row][col].R = ((double)(px[0]))/255.0;
			img -> pixels[row][col].G = ((double)(px[1]))/255.0;
			img -> pixels[row][col].B = ((double)(px[2]))/255.0;
		}
  ...
  for(int row = 0; row < img -> height; row++)
	{
		...
		{
			uint8_t* px = &(row_pointers[row][col * 4]);
			px[0] = (uint8_t)(img -> pixels[row][col].R * 255.0);
			px[1] = (uint8_t)(img -> pixels[row][col].G * 255.0);
			px[2] = (uint8_t)(img -> pixels[row][col].B * 255.0);
			px[3] = 255;
		}
  }
